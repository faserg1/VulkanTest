...
int main()
{
	...
	*/
	 ...
	 * VK_FENCE_CREATE_SIGNALED_BIT, который приведёт наш забор в сигнальное состояние.
	 * pNext должен быть NULL.
	*/
	fence_create_info.flags = 0; //но всё же оставим NULL
	 ...
	 * Если наш забор уже изначально в сигнальном состоянии, эта функция нас сразу же пропустит.
	 ...
	 * чего-то крупного. Если забор уже просигнален, то timeout роли не играет.
	 * Если истечёт время — VK_TIMEOUT, если забор просигнален — VK_SUCCESS.
	 * Кстати, если этот забор никуда не посылали, то нам должен дебаг репорт выдать об этом предупреждение.
	 * Отошлём один из буферов и подождём его выполнение с помощью забора.
	 * Время истечения зададим - бесконечность, т.е самое большое число считается бесконечным ожиданием.
	*/
	vkQueueSubmit(queue, 1, &submit_info[0], fence);
	if (vkWaitForFences(r.GetDevice(), 1, &fence, false, (uint64_t) -1) != VK_SUCCESS)
		return 2;
