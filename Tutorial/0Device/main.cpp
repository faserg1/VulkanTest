...
int main()
{
	...
	* На момент написания используется версия 1.0.3 (если кому интересно :D ).
	* 
	...
	*/
	 ...
	 * изменения, но и не маленькие. И псоледняя — patch. Маленькие патчи, фиксы
	 * и всё что в этом духе. Примерно так и спользуется версии в Vulkan.
	...
	*/
	 ...
	 * макрос VK_NULL_HANDLE, который заменяет привычный NULL. Сам хэндл сам по себе — 
	 * указатель на структуру, как это показывается в vulkan.h.
	*/
	 ...
	 * Vulkan, а точнее тем, как он распределяет память — сдлеайте свой обработчик.
	 * Подробнее о том, как его сделать — напишу в следующих уроках.
	*/
	create_instance_result = vkCreateInstance(&instance_info, VK_NULL_HANDLE, &instance);
	...
	*/
	 ...
	 * возможности (capabilities) тоже. В-третьих, получить специальные указатели на фнукции, которые
	 ...
	 * и устройство. Поэтому прежду чем уничтожить экземпляр, нужно уничтожить все устрайства, созданные этим 
	 ...
	 * Предупреждение: если вам уже сейчас надоело  — бросайте. Дальше только гуще.
	 *
	 * Теперь немного о устройстве. В Vulkan есть разделение на физическое устройство, так и на логическое.
	 * Примерно как с жёскими дисками: есть как физический диск (наша реальная коробочка с данными внутри другой
	 * коробочки с много чем ещё) и логический. Причём, логических дисков может быть несколько на одном физическом.
	 * Тоже самое и с Vulkan. Сначала мы узнаем подробности о физическом устройстве, и только потом создаём "на нём"
	 * логическое.
	 * 
	 * Для создания логического устройства нам необходимо будет указать следующее: какие в нём будут слои,
	...
	*/
	 ...
	 * Но сейчас мы не будем отправлять VK_NULL_HANDLE в 3 аргумент, а второй аргумент уже не
	 * выходной, а входной параметр, т.е. pPhysicalDeviceCount — сколько наш массив может выдержать
	...
	*/
	 ...
	 * 
	 * Ну как? Асфальт кипит? Если РЕАЛЬНО всё понятно, то можно идти дальше. Иначе — советую переварить всё
	 * вышесказанное про семейства и их возможностях.
	 * 
	...
	*/
	...
	/*Теперь кое-что поинтересней. У очередей есть приоритет выполнение, т.е, какой из очереди
	...
	*/
	...
	device_queue_info.queueCount = 1; //укажем кол-во очередей, которые нам нужно, сейчас нам хватит и одной.
	...
	*/ 
	...
	/* Указываем кол-во.. нет, не очередей =) Структур, которые потенциально содержат ифнормацию о очередях и их количестве.
	...
	*/
	 ...
	 * полученную из vkGetPhysicalDeviceFeatures. Если вы хотите включить только необходимые (requred) фичи,
	 * отправьте NULL.
	*/ 
	...
	*/
	 ...
	 * Да теперь мы можем просто отправить пачкой команды в видеокарту, а не каждый раз теребить по одной команде
