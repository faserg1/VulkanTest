...
int main()
{
	...
	* версии API. На самом деле, apiVersion можно оставить NULL, тогда оно будет игнорироваться.
	...
	* Обычно, драйвер не считает версию несовместимой, если патч версия выше, чем указано в драйвере,
	* но первые (бета) версии драйвера NVIDIA выдавали ошибку.
	*
	* Также, раньше существовал макрос VK_API_VERSION, теперь он стал устревшим (deprecated,
	* "осуждённый", "неодобрённый"), как раз из-за того, что в приницпе особой роли патч версия не играет.
	* Поэтому появился макрос VK_API_VERSION_1_0.
	*/
